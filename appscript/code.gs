/**
 * Akhila / PCOS Reset — downstream Meta CAPI event firing engine.
 *
 * Bound to the "CRM" tab of the Akhila CRM Google Sheet. On every USER edit
 * to a trigger column (call_booked / call_showed / sale_closed) this script
 * reads the lead's row, builds a Meta Conversions API event payload with full
 * hashed user_data, POSTs to graph.facebook.com directly, and stamps the row
 ...
 * ~15 min by copy-pasting this file + setting three script properties.
 *
 * Schema: 36-column row A..AJ. Tab name "CRM". See readme.md.
 */
...
const MAIN_SHEET_NAME_DEFAULT = 'CRM';
...
const COL = {
  ...
  // --- Manual / Apps Script-written (X..AJ) ---
  CALL_BOOKED:               24,  // X
  BOOKING_TIME:              25,  // Y
...
function postToMetaCapi(payload) {
  ...
  // Optional: route to Test Events while validating (Events Manager → Test
  // Events → copy code). Leave UNSET in production.
  const testEventCode = props.getProperty('META_CAPI_TEST_EVENT_CODE');
...
function setupTriggers() {
  ...
  console.log(msg);
  SpreadsheetApp.getActive().toast(msg, 'Akhila CRM', 5);
}
...
function replayPendingEvents() {
  ...
  SpreadsheetApp.getActive().toast('Replayed ' + replayed + ' pending event(s)', 'Akhila CRM', 5);
}
